...
class Class_FCCDN_loss_BCD(nn.Module):
    def __init__(self):
        super(Class_FCCDN_loss_BCD,self).__init__()
        self.criterion=dice_bce_loss()
    def forward(self,scores,labels):
        loss_change = torch.tensor(0.0).cuda()
        loss_aux = torch.tensor(0.0).cuda()
        # change loss
        loss_change = self.criterion(scores[0], labels[0])
        # seg map
        ...
        # seg loss
        loss_aux = 0.2 * self.criterion(out1, pred_seg_post_tmp1, False)
        loss_aux += 0.2 * self.criterion(out2, pred_seg_pre_tmp1, False)
        loss_aux += 0.2 * self.criterion(out3, labels[1] - pred_seg_post_tmp2, False)
        loss_aux += 0.2 * self.criterion(out4, labels[1] - pred_seg_pre_tmp2, False)
